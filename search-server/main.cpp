        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) {
                if constexpr (is_same_v<DocumentPredicate, DocumentStatus>) {
                    if (documents_.at(document_id).status == document_predicate) {
                        document_to_relevance[document_id] += term_freq * inverse_document_freq;
                    }
                }
                else if (document_predicate(document_id, documents_.at(document_id).status, documents_.at(document_id).rating)) {
                    document_to_relevance[document_id] += term_freq * inverse_document_freq;
                }
            }
        }
