...
void TestFindAddedDocumentByDocumentWord() {
...

    {
        ...
        server.AddDocument(doc_id + 1, "the dog goes home", DocumentStatus::ACTUAL, ratings);
...
void TestExcludeDocumentsWithMinusWordsFromResults() {
    ...
    server.AddDocument(2, "red dog goes home "s, DocumentStatus::ACTUAL, { 1,2,3 });

    {
        const auto found_docs = server.FindTopDocuments("red -dog"s);
        ASSERT_EQUAL(found_docs.size(), 1);
