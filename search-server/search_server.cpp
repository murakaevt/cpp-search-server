...
void SearchServer::AddDocument(int document_id, const std::string& document, DocumentStatus status,
    ...
    const auto words = SplitIntoWordsNoStop(document);                                           //вектор слов к добавлению
...
void SearchServer::RemoveDocument(int document_id) {
    ...
    for (auto [key_word, _] : id_word_freqs_.at(document_id)) {                // перебираем ключи-слова внутреннего словаря
        ...
        word_to_document_freqs_.at(key_word).erase(document_id);               // и удаляем id из внутреннего словаря 
        if (word_to_document_freqs_.at(key_word).empty()) {                    // если ключу теперь соответствует пустое значение
            word_to_document_freqs_.erase(key_word);                           // удаляем внешний ключ
        }
    }
...
std::vector<Document> SearchServer::SearchServer::FindTopDocuments(const std::string& raw_query) const {
...
std::tuple<std::vector<std::string>, DocumentStatus> SearchServer::MatchDocument(const std::string& raw_query,
    ...
    // LogDuration guard("Operation time", std::cout);
